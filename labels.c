...
public  VIO_Volume  create_label_volume(
    VIO_Volume  volume,
    nc_type type )
{
...
public  VIO_BOOL  is_label_volume_initialized(
    VIO_Volume  volume )
{
    return( volume != NULL && 
...
public  VIO_Status  load_label_volume(
    VIO_STR   filename,
    VIO_Volume   label_volume )
{
    int                   slice, n_slices;
            ...
            for_inclusive( label_voxel[Y], limits[0][Y], limits[1][Y] )
            {
                if( is_linear )
                {
                    if( label_voxel[Y] == limits[0][Y] )
                    {
                        ...
                        if( is_integer_step )
                        {
                            int_y_voxel[X] = VIO_FLOOR( y_voxel[X] );
                            int_y_voxel[Y] = VIO_FLOOR( y_voxel[Y] );
                            int_y_voxel[Z] = VIO_FLOOR( y_voxel[Z] );
                        }
                    }
                    else
                    {
                ...
                for_inclusive( label_voxel[Z], limits[0][Z], limits[1][Z] )
                {
                    ...
                    if( !is_integer_step )
                    {
                        int_voxel[X] = VIO_FLOOR( voxel[X] );
                        int_voxel[Y] = VIO_FLOOR( voxel[Y] );
                        int_voxel[Z] = VIO_FLOOR( voxel[Z] );
                    }
...
public  VIO_Status  save_label_volume(
    VIO_STR   filename,
...
public  VIO_Status  input_tags_as_labels(
    FILE    *file,
    VIO_Volume  volume,
    VIO_Volume  label_volume )
{
...
public  VIO_Status  create_label_volume_from_file(
    VIO_STR   filename,
    VIO_Volume   volume,
    VIO_Volume   *label_volume )
{
...
public  VIO_Status  output_labels_as_tags(
    FILE    *file,
    VIO_Volume  volume,
    VIO_Volume  label_volume,
    int     desired_label,
...
public  VIO_Status  input_landmarks_as_labels(
    FILE    *file,
    VIO_Volume  volume,
    VIO_Volume  label_volume )
{
