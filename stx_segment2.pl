...
#@VERSION    : $Id: stx_segment2.pl,v 1.3 2005-05-26 20:13:29 bert Exp $
#-----------------------------------------------------------------------------
...
if (defined($Skull_file) && -e $Skull_file) {
    if ($Clobber) {
	unlink($Skull_file);
    }
    else {
...
($grey, $white, $csf, $bkg) = &separate_classes( $classified, $Template );
                                ...
                                # segment out the skull if required.

($skull_atlas)  = &customize_atlas($SkullAtlas , $def_inv, $Template, "skull_atlas");
($skull) = &merge_atlas    ( $skull_atlas,  $bkg, "mask", "skull");

if (defined ($Skull_file)) {
...
#($vent_template)= &build_smallest_template($VenLimits, $csf, $def_inv);
#($vent )        = &customize_atlas($VenAtlas ,  $def_inv, $vent_template,"vent_atlas");
...
($custom_atlas3) = &merge_atlas($custom_atlas2, $skull,          "over", "with_vent+BG+skull");
($custom_atlas4) = &merge_atlas($custom_atlas3, $white_atlas,    "under","final");


if (defined ($SurfaceObj)) {
   print ("Creating mask...\n");
   ($mask) = build_brain_mask($SurfaceObj, $custom_atlas3);
   print ("done mask.\n");
   ...
   &Spawn("cp $custom_atlas5 $outputfile");
                                ...
                                # Cerb_r and BS from the custum_grey_atlas
...
sub merge_atlas {
  ...
  if (-e $custom) {
    ...
    if ($operation eq "like") {
      ...
      &Spawn("$MincMask -invert_mask $first $second $tmp");      
    }
    elsif ($operation eq "mask") {
      &Spawn("$MincMask $first $second $tmp");      
    }
...
sub build_template {
  ...
  if (-e $custom) {
    print "file $custom exists already\n";
     ...
     &Spawn("$AutoCrop  -isoexpand 5% $template $custom -from $tags");
  }
...
sub customize_atlas {
  ...
  if (-e $custom) {
    ...
    &Spawn("$MincResample $atlas $custom -like $template -transformation $xfm -nearest");
  }
  ($custom)
}
...
sub discrete_segment  {
    my ($atlas,$mask,$output) = @_;

    if (-e $output) {
      print "file $output already exists\n";
    }
    else {
      &Spawn("$MincMask $atlas $mask $output");
    }
}
...
sub separate_classes  {
	...
	print ("file $grey already exists\n");
        ...
        $llimit = $GWCB[0] - 0.5;
        $ulimit = $GWCB[0] + 0.5;
	&Spawn("$Mincmath $temp $grey -seg -const2 $llimit $ulimit");
    }
	...
	print ("file $white already exists\n");
        ...
        $llimit = $GWCB[1] - 0.5;
        $ulimit = $GWCB[1] + 0.5;
	&Spawn("$Mincmath $temp $white -seg -const2 $llimit $ulimit");
    }
	...
	print ("file $csf already exists\n");
        ...
        $llimit = $GWCB[2] - 0.5;
        $ulimit = $GWCB[2] + 0.5;
	&Spawn("$Mincmath $temp $csf -seg -const2 $llimit $ulimit");
    }
	...
	print ("file $bkg already exists\n");
        ...
        $llimit = $GWCB[3] - 0.5;
        $ulimit = $GWCB[3] + 0.5;
	&Spawn("$Mincmath $temp $bkg -seg -const2 $llimit $ulimit");
    }
...
sub prepare_transforms {
  ...
  if (-e $forward_rev) {
    print "file $forward_rev already exists.\n";
      ...
      my $inverted_xfm = "${TmpDir}/inverted.xfm";
      &Spawn("xfminvert $xfm $inverted_xfm");
      &Spawn("$Xfm2Def $inverted_xfm $forward_rev");
      exit(1);
...
as the fourth argument and the skull labels file must be explicitly
specified with the '-skull_labels' option. See the example below:
...
between the input MRI volume and the atlas labels.  This script takes three
arguments:
...
HELP
  ...
  $Atlas_source = "icbm152-v1.0"; # use the standard ICBM atlas by default

  undef $Skull_file;
  undef $Only_skull_file;
  @GWCB = ( 2, 3, 1, 0 );
  ...
  @ArgInfo =
    (@DefaultArgs,
     ...
     ["-skull", "string", 1, \$Skull_file,
        "specify file to store skull labels (.mnc)"],
     ["-only_skull", "boolean", undef, \$Only_skull_file,
        "segment labels for the skull, then quit"],
     ["-original_atlas", "copy", "icbm152-v1.0", \$Atlas_source,
        "use original ICBM152 probabilistic atlas (default)"],
     ["-symmetric_atlas", "copy", "icbm152-symmetric-v1.0", \$Atlas_source,
        "use symmetric ICBM152 probabilistic atlas (strongly suggested)"],
     ["-gwcb", "integer", 4, \@GWCB,
   ...
   if (defined ($Only_skull_file) ) {
     ...
     if (! $Skull_file) {
       warn $usage3;       
       die "Need to specify the file for skull labels with '-skull' option.\n";
     }
   ...
   if (@argv != $arg_count )  {
     if ($arg_count==4) {
       ...
       warn $usage3;       
       if (@argv == 4) {
	 die "You can't specify the labels file as the 4th argument.\n";
       }
     }
   ...
   $VenAtlas         = "${Modeldir}atlas_ven_manual.mnc";
   $SkullAtlas       = "${Modeldir}atlas_skull.mnc";
   ...
   $MincMask     .= "$debug$verbose$clobber";
   $Mincmath     .= "$debug$verbose$clobber";
   
   if ($arg_count == 3) {
      ($transform, $stx_xfm, $classified) = @argv;
   }
   else {
