...
#ifndef lint
static char rcsid[] = "$Header: /private-cvsroot/libraries/conglomerate/thin_plate_spline.c,v 1.1 2001-05-11 08:11:25 stever Exp $";
#endif
    ...
    Real    **derivs )
{
    ...
    for_less( p, 0, n_points )
    {
        ...
        weight_ptr = weights[p];
        for_less( v, 0, n_values )
            values[v] += weight_ptr[v] * dist;
    ...
    for_less( d, 0, n_dims )
    {
        weight_ptr = weights[n_points+1+d];
        this_pos = pos[d];
        for_less( v, 0, n_values )
        {
            values[v] += weight_ptr[v] * this_pos;
            if( derivs != NULL )
                derivs[v][d] += weight_ptr[v];
        }
    }
}
